...
switch(_style)
	...
	case HIT_FX.normal_medium:
		//Direction Burst
		var _fx = fx_create(spr_hit_fx_direction_blast, 1, 0, 27, _hit.x, _hit.y, 1, _angle, _layer);
		_fx.fx_ys *= choose(-1, 1);
	...
	case HIT_FX.slash:
		...
		var _fx = fx_create(spr_hit_fx_direction_burst, 1, 0, 13, _hit.x, _hit.y, 1, _angle, _layer);
		_fx.fx_ys *= choose(-1, 1);
		var _fx = fx_create(spr_hit_fx_slash, 1, 0, 15, _hit.x, _hit.y, 1.5, _angle, _layer);
		...
		with(_hit)
			...
			part_particles_create(global.part_sys, x + (sprite_get_width(anim_sprite) / 2), y + (sprite_get_height(anim_sprite) / 2), global.p1, max(_knock / 5, 5));
